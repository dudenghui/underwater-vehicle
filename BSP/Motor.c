...
/******************************************************************************
函数原型：	static void Tim3_init(void)
功    能：	Tim3初始化
*******************************************************************************/ 
static void Tim3_init(void)
{
	...
	TIM_TimeBaseStructure.TIM_ClockDivision = 0;        //设置时钟分割:TDTS = Tck_tim
	TIM_TimeBaseStructure.TIM_CounterMode = TIM_CounterMode_Up; //TIMx向上计数模式
	TIM_TimeBaseInit(TIM3, &TIM_TimeBaseStructure);     //根据TIM_TimeBaseStructure中指定的参数初始化外设TIM3
...
void Motor_Init(void)
{
  ...
  //设置电机使用到得管脚
	GPIO_InitStructure.GPIO_Pin = GPIO_Pin_0 | GPIO_Pin_1 | GPIO_Pin_6 | GPIO_Pin_7 | GPIO_Pin_8 | GPIO_Pin_9 ; //对应Tim4
...
//使能电机用的时钟
  ...
  //设置电机使用到得管脚
	GPIO_InitStructure.GPIO_Pin = GPIO_Pin_6| GPIO_Pin_7; //对应Tim3
...
void Motor(int16_t duty1,int16_t duty2,int16_t duty3,int16_t duty4,int16_t duty_l,int16_t duty_r,int16_t duty_sevro,int16_t duty_light)
{
	...
	TIM4->CCR1 = 1500+duty1;           //B6TIM2->CCR1 = 1500+duty_l;           //A0
	TIM4->CCR2 = 1500+duty2;           //B7
	TIM4->CCR3 = 1500+duty3;           //B8
	TIM4->CCR4 = 1500+duty4;           //B9
	TIM3->CCR1 = 1500+duty_l;           //A1
	TIM3->CCR2 = 1500+duty_r;           //A1s
}
/******************************************************************************
