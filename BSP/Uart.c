...
volatile uint8_t Tx_Buff1[20],Tx_Buff2[5];	
volatile uint8_t Rx_Buffer1[20],Rx_Buffer2[10];//串口接收区
volatile uint8_t com_data1;//串口1接收数据缓存区
volatile uint8_t com_data2;//串口3接收数据缓存区

volatile uint8_t USART_TX_BUF[20];//串口发送区
...
void USART1_IRQHandler(void) //串口1中断服务程序       控制信号接受
{
  ...
  com=1;
	if(USART_GetITStatus(USART1, USART_IT_RXNE) != RESET)  //接收中断(接收到的数据必须是0x0d 0x0a结尾)
	{
		...
		else if(state_machine1 == 1)//接受控制数据Rx_Buffer1[1]-Rx_Buffer1[6]依次为左x,左y,右x,右,hh
			   {   
				   Rx_Buffer1[lencnt1++] = Res; // 数据保存
				   if(lencnt1>=6) //数据接收完毕
				   { 
	   ...
	   if(state_machine2 == 0)// 协议解析状态机
		{     
			...
			{	
			   ...
			   {     
				   if((Res==0x51)||(Res==0x52)||(Res==0x53))// 接收到帧头第二个数据  
				   {   
						 state_machine2 = 2;     
						 Rx_Buffer2[lencnt2++] = Res;// 数据保存   
				   }
